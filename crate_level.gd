...
func _on_delivery_area_body_entered(body):
	if body == $crate: #TO DO: insert level object
		$fork_lift.control = false
...
func _on_hud_next():
	get_tree().change_scene_to_file("res://fridge_level.tscn") #TO DO:insert next level path
...
func _on_floor_is_lava_body_exited(body):
	if body == $crate : #TO DO:insert_level_object:
		begin = true


func _on_floor_is_lava_body_entered(body):
	if begin && body == $crate: #TO DO:insert level object:
		lose()
